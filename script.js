...
var w = document.getElementById("small_canvas").getAttribute("width");
var h = document.getElementById("small_canvas").getAttribute("height");
...
const draw = (timestamp, vid) => {
    ...
    window.requestAnimationFrame((timestamp) => draw(timestamp, vid))
}
...
navigator.mediaDevices.getUserMedia(options)
    .then(function (stream) {  // Main function
        video.srcObject = stream
        Tone.start()
        // window.requestAnimationFrame((timestamp) => draw(timestamp, video))
    })
    .catch((error) => {
